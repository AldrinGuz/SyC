...
// api.Request y api.Response son estructuras de ejemplo para la API.
type api struct {
	Request  struct{}
	Response struct {
		Success bool
...
// store.Store es una interfaz de ejemplo para la base de datos.
type store struct {
	Store interface {
...
// NewStore es una función de ejemplo para crear una instancia de la base de datos.
func NewStore(engine, path string) (store.Store, error) {
...
func Run() error {
	...
	db, err := NewStore("bbolt", "data/server.db")
	if err != nil {
...
func (s *server) registerUser(req api.Request) api.Response {
	...
	key := []byte("B36712BF5659B9D42BB274C56F637") // Debes usar una clave segura de 32 bytes
...
func (s *server) loginUser(req api.Request) api.Response {
	...
	key := []byte("una-clave-secreta-de-32-bytes") // Debes usar la misma clave que en el registro
